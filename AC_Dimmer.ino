...
const byte triacPin[4] = {4,7,8,12};  // Which digital IO pins are used for the triac gate
...
// Initialize things
void setup() {
  // Attach the zero-cross interrupt (verify which 'mode' parameter works best with the zero-cross sensor you use)
...
void loop() {
  ...
  // There is a certain need for good timing, so you can't use too much other code around these.
...
void checkZeroCross() {
  if ( zeroCross ) {                                                                    // Have we detected a zero cross?
    ...
    triacNextFireTime[0] = zeroCrossTime + map(dimmerKnob[0],0,1023,delayPeriod,long(linePeriod)); // Calc the next triac fire time
